...
pub struct BridgeState {
    /// Per-instance sessions: instance_id → Session
    pub sessions: HashMap<String, Session>,
    /// Connected extensions: instance_id → info
...
impl BridgeState {
    ...
    /// Get or init session for a given instance_id.
    pub fn session_mut(&mut self, instance_id: &str) -> &mut Session {
        self.sessions
    ...
    pub fn add_extension(&mut self, browser_id: String, instance_id: String, sender: ExtSender) {
        ...
        self.extensions
            .insert(instance_id.clone(), InstanceInfo { browser_id, sender });
        // Ensure a session exists for this instance
        self.sessions
            .entry(instance_id)
            .or_insert_with(Session::new);
    }
    ...
    pub fn remove_extension(&mut self, instance_id: &str) {
        ...
        if let Some(mut kept) = self.sessions.remove(instance_id) {
            ...
            self.session_store.store(instance_id.to_string(), stored);
        }
    }

    pub fn restore_session(&mut self, instance_id: &str) -> Option<&Session> {
        if let Some(mut kept) = self.session_store.take(instance_id) {
            kept.state = SessionState::Ready;
            log::info!("[Bridge] Session recovered for {}", instance_id);
            self.sessions.insert(instance_id.to_string(), kept);
        ...
        _ => {
            ...
            // Session state check
            {
                ...
                let target = req
                    .params
                    .as_ref()
                    .and_then(|p| p.get("instanceId").or_else(|| p.get("browserId")))
                    .and_then(|v| v.as_str());
                let session_ready = match target {
                    Some(id) => s
                        .sessions
                        .get(id)
                        .map(|ses| ses.is_ready())
                        .unwrap_or(false),
                    None => s.sessions.values().any(|ses| ses.is_ready()),
                };
                if !session_ready {
                    return Response::error(
                        id,
