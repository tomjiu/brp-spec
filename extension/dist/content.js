...
(() => {
  ...
  // src/content.ts
  var itree = window.__BRP_ITREE__;
  ...
  var SENSITIVE_INPUT_TYPES = /* @__PURE__ */ new Set(["password", "hidden"]);
  ...
  function isSensitiveElement(el) {
    ...
    const name = el instanceof HTMLElement && "name" in el && typeof el.name === "string" ? el.name : el.getAttribute("name") ?? "";
    const id = el.id ?? "";
    const placeholder = el.getAttribute("placeholder") ?? "";
    const nameIdPlaceholder = [name, id, placeholder].join(" ").toLowerCase();
    return SENSITIVE_KEYWORDS.some((kw) => nameIdPlaceholder.includes(kw));
  }
  ...
  function doSelect(msg) {
    ...
    const values = msg.values ?? (msg.value !== void 0 ? [msg.value] : []);
    if (values.length === 0) {
  ...
  function doGetAttribute(msg) {
    ...
    const sensitiveAttrs = /* @__PURE__ */ new Set(["value", "textContent", "innerText", "innerHTML"]);
  ...
  function doWaitForSelector(msg) {
    const selector = msg.css ?? (typeof msg.selector?.value === "string" ? msg.selector.value : void 0);
    if (!selector) return Promise.resolve({ error: "No CSS selector provided" });
    const timeout = msg.timeout ?? 1e4;
